...
import { X, Share2, Mail, CheckCircle, Copy } from 'lucide-react';
...
const ShareFileModal: React.FC = () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const link = `https://campusshare.ai/view/${fileToShare.id}/${Math.random().toString(36).substring(2, 10)}`;
        setShareLink(link);
        setView('success');
    };
    ...
    const renderSuccess = () => (
        <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Share Link Generated</h2>
            <p className="text-slate-600 dark:text-gray-300 mb-6">
                Anyone with this link can view "<span className="font-medium text-slate-700 dark:text-gray-200">{fileToShare.name}</span>".
            </p>
            <div className="relative mb-6">
                <input
                    ...
                    value={shareLink}
                    readOnly
                    className="w-full pr-28 pl-4 py-2.5 bg-slate-100 dark:bg-slate-900/50 border border-slate-300 dark:border-blue-500/30 rounded-lg text-slate-900 dark:text-white"
                />
                <button
                    onClick={handleCopy}
                    className="absolute right-1.5 top-1/2 -translate-y-1/2 px-4 py-1.5 bg-blue-600 text-white rounded-md font-semibold hover:bg-blue-700 transition flex items-center space-x-2 text-sm"
                >
                    {copied ? <CheckCircle className="w-4 h-4" /> : <Copy className="w-4 h-4" />}
                    <span>{copied ? 'Copied!' : 'Copy'}</span>
                </button>
            </div>
            <button onClick={handleCancel} className="w-full max-w-xs mx-auto px-6 py-3 bg-slate-200 dark:bg-slate-700/50 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-white rounded-lg font-semibold transition">
                Done
            </button>
        </div>
    );
